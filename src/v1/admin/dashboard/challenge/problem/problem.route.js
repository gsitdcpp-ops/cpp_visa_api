const baseRoute = "challenge/problem";
const model = require("./problem.model");
const modelChallageType = require("../type/type.model");
...
const route = (prop) => {
  ...
  prop.app.post(
    ...
    async (req, res) => {
      try {
        const { tittle, article, village_id, challenge_type_id } = req.body;
        ...
        if (!challenge_type_id) {
          return res.status(400).json({
            success: false,
            message: "មិនមានទិន្នន័យមិនត្រឹមត្រូវ (challenge_type_id)!",
          });
        }

        if (challenge_type_id) {
          if (!mongoose.Types.ObjectId.isValid(challenge_type_id)) {
            return res.status(400).json({
              success: false,
              message: "មិនមានទិន្នន័យមិនត្រឹមត្រូវ (challenge_type_id)!",
            });
        ...
        const challenge = await modelChallageType.findOne({
          _id: challenge_type_id,
          deleted: false,
        });
        ...
        if (!challenge) {
          return res.status(400).json({
            success: false,
            message: "មិនមានទិន្នន័យមិនត្រឹមត្រូវ (challenge id)!",
          });
        }
        ...
        data.challenge_type_id = challenge_type_id;
        ...
        if (filesThumbnail) {
          const uploadThumb = await cloudinary.uploader.upload(
            filesThumbnail.path,
            { folder: "problem_posted/thumbnail" }
          );
    ...
    async (req, res) => {
      try {
        ...
        function extractPublicId(url) {
          ...
          let path = parts[1];
          // Example: v1733300000/problem_posted/group/xyz778.jpg
  ...
  prop.app.get(
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (challenge_type_id == "all") {
        } else {
          if (!mongoose.Types.ObjectId.isValid(challenge_type_id)) {
            return res.status(400).send({
              success: false,
              message: "challenge_type_id មិនត្រឹមត្រូវ!",
            });
          }
        }
  ...
  async function getPaginations(
    ...
    if (challenge_type_id == "all") {
      ...
      mongoFilter.challenge_type_id = new mongoose.Types.ObjectId(
        challenge_type_id
      );
    }
